...
class generalFunctions:
    ...
    def writeDataFile (self, sensorDirPath, saveFileName, sensor_readings, dataFrame=True, headers=None): 
        # SheetName='Sensor_'+str(sensorN)
        # csvfilename='Parsa360sensor_readings'+str(SheetName)+'.csv'
        txtfilename=str(saveFileName)+'.txt'
            
        # csvfile=os.path.join(dataLogstorePath, csvfilename)
        ...
        if not os.path.isfile(txtfile) and not headers==None:
            ...
            if dataFrame==True:
                ...
                with open(txtfile, mode='w') as sensorReadingFile:
                    ...
                    # sensorReadingFile.write(headers + "\n")

        if os.path.isfile(txtfile):
            print(os.path.abspath(txtfile))
        ...
        if dataFrame==True:
            ...
            # sensorReadingFile.write(" ,".join(str(item) for item in sensor_readings))
            # return(write_to_log)
        # with open(txtfile, mode="a") as myfile: 
        #     myfile.write(sensor_readings + "\n")
